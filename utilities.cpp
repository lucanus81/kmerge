...
seastar::future<long unsigned int> create_test_file_async(const char* path, size_t chunks) {
  ...
  const size_t BUFFER_SIZE{chunks * file::FILESIZE};
  auto wbuf = seastar::temporary_buffer<char>(BUFFER_SIZE);
  char* pos = wbuf.get_write();
  for (size_t i=0; i<chunks; ++i) {
    file* f = new (pos) file{d(gen)};
    std::cout <<"written key " <<f->key() <<'\n';
    pos += file::FILESIZE;
  ...
  return seastar::with_file(
    seastar::open_file_dma(path, seastar::open_flags::rw | seastar::open_flags::create), [&wbuf] (seastar::file& f) {
      return f.dma_write(0, wbuf.get(), wbuf.size());});
}
